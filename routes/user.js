...
router.get('/view-products', ensureAuthenticated, async (req, res) => {
  try {
    ...
    // Fetch products using the productHelper or directly from the database
    const products = await productHelper.getAllProducts();

    // Check for a success message in the query parameters
    const successMessage = req.query.successMessage || null;

    // Render the view-products page with products, user data, success message, and admin flag
    res.render('user/view-products', {
      products,
      user,
      successMessage,
      admin: false, // Set admin to false as per your previous route
    });
  } catch (error) {
...
router.get('/', async (req, res) => {
  try {
    if (req.session.loggedIn) { // Check if the user is logged in
      let user = req.session.user; // Get user details from session
      let products = await productHelper.getVisibleProducts(); // Fetch only visible products

      // Pass the user data (including imagePath) and products to the homepage template
      res.render('user/view-products', { 
        products, 
        user, // Include the entire user object in the template
      });
    } else {
    ...
    console.error('Error fetching products:', error);
    res.redirect('/'); // Redirect to the homepage if there's an error
  }
});
...
router.get('/reactivate-product/:id', (req, res) => {
  ...
  productHelper.reactivateProduct(productId)
    .then(() => {
      req.session.successMessage = 'Product reactivated successfully!';
      res.redirect('/admin'); // Redirect back to the admin page
    })
    .catch((err) => {
      console.error('Error reactivating product:', err);
      req.session.errorMessage = 'Failed to reactivate product.';
      res.redirect('/admin');
    });
});
...
router.get('/addtocart', async (req, res) => {
  try {
    ...
    if (!user || !productId || !productName) {
      return res.status(400).json({ error: "User, Product ID, and name are required" });
    }

    const result = await cartHelpers.addProductToCart(user, productId, productName);
...
router.get('/cart', async (req, res) => {
  ...
  const { products, cart, totalPrice } = await cartHelpers.getUserCartWithDetails(user._id);

  // Convert cart to a JSON string before sending it to the view
  const cartJson = JSON.stringify(cart);

  res.render('user/cart', {
    user,
    products,
    cart: cartJson,  // Send cart as a JSON string
    totalPrice,
    admin: false,
  });
});
...
router.post("/cart-remove", async (req, res) => {
  const userId = req.session.user; // Assuming user session stores the logged-in user's ID
  const { productId } = req.body;

  try {
    // Call the helper function to remove the product from the cart
    const result = await cartHelpers.removeProductFromCart(userId, productId);

    if (result.cartEmptied) {
      // If the cart is now empty, send a response indicating so
      return res.status(200).json({ message: "Cart is now empty." });
    }

    // If the product was successfully removed but the cart still has items
  ...
  try {
    ...
    await db.get()
      ...
      .updateOne(
        ...
        { $set: { orderStatus: 'Confirmed' } }  // Update the status to 'Confirmed'
      );
