...
>>>>>>> 0716428c1e8858e0d80e087156e661d93a2f21fd
  ...
  router.post('/edit-product/:id', async (req, res) => {
    ...
    const newImage = req.files?.image; // Check if a new image is uploaded
  
    try {
      // Call the updateProduct function with the updated data and new image
...
router.get('/toggle-show/:id', async (req, res) => {
  ...
  try {
    // Fetch the product to get the current `isShown` status
    const product = await productHelper.getProductById(productId);
    ...
    // Toggle the `isShown` property
    const updatedShowStatus = !product.isShown;

    // Update the product in the database
    await productHelper.updateProduct(productId, { isShown: updatedShowStatus });

    console.log(`Product ${productId} visibility toggled to: ${updatedShowStatus}`);
    res.redirect('/admin'); // Redirect back to the admin products page
  } catch (error) {
